...
int main(int argc, char *argv[]) {
  ...
  fprintf(outfile, "#ifndef _SEROKUCHI_FONT%dX%d_H_\n#define _SEROKUCHI_FONT%dX%d_H_\n\n",width/16,height/6,width/16,height/6);
  fprintf(outfile, "#include <stdint.h>\n#include <string.h>\n\n");
  fprintf(outfile, "static uint8_t font%dx%d[%d][%d] = {\n",width/16,height/6,height,width);
  for(int i = 0; i < height; i++){
    fprintf(outfile, "{");
    for(int j = 0; j < width; j++){
      fprintf(outfile, "0x%02X%s", reverse_rows[i][j*4],j == width - 1 ? "" : ",");}
    fprintf(outfile, "}%s\n",i == height - 1 ? "" : ",");}
  fprintf(outfile, "};\n\n");
  fprintf(outfile, "void DrawString%dx%d(int32_t x, int32_t y, const char* text, uint32_t color, uint32_t scale, int32_t spacing_x, int32_t spacing_y){\n",width/16,height/6);
  ...
  fprintf(outfile, "      text_x = 0; text_y -= %d * scale + spacing_y;}\n", height/6);
  ...
  fprintf(outfile, "            if(font%dx%d[k + char_index_y * %d][j + char_index_x * %d])\n",width/16,height/6,height/6,width/16);
  ...
  fprintf(outfile, "                  D(x + text_x + (j * scale) + scaled_i, y + text_y + (k * scale) + scaled_j, color);}\n",width/16);
  fprintf(outfile, "      else{\n");
  fprintf(outfile, "        for(uint32_t j = 0; j < %d; j++)\n",width/16);
  fprintf(outfile, "          for(uint32_t k = 0; k < %d; k++)\n",height/6);
  fprintf(outfile, "            if(font%dx%d[k + char_index_y * %d][j + char_index_x * %d])\n",width/16,height/6,height/6,width/16);
  fprintf(outfile, "              D(x + text_x + j, y + text_y + k, color);}\n",width/16);
  fprintf(outfile, "      text_x += (%d + spacing_x) * scale ;}}\n",width/16);
  fprintf(outfile, "}\n\n");
